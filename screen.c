...
int screen_gameover(Camera2D camara, Font fuente){
    float start_x = camara.target.x - (3 * TILE);
    float start_y = camara.target.y - (1.5 * TILE);
    float start_width = (6 * TILE) + 7;

    float quit_x = start_x;
    float quit_y = camara.target.y + (.5 * TILE);
    float quit_width = (6 * TILE) + 7;

    Vector2 vec_start = {camara.target.x - (3 * TILE) + 2, camara.target.y - (1.5 * TILE) + 2};
    Vector2 vec_quit = {camara.target.x - (3 * TILE) + 2, camara.target.y + (.5 * TILE) + 2};
    ...
    while (!WindowShouldClose()) {
        ...
        DrawTextEx(fuente, " Nueva Partida", vec_start, 10, .05, BLACK);
        DrawTextEx(fuente, "     Salir    ", vec_quit, 10, .05, BLACK);
